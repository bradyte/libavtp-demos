/*
 * AAF Listener — Raspberry Pi 5 (CLOCK_REALTIME)
 *
 * Receives IEEE 1722 AAF packets and plays via ALSA with presentation-time
 * gating. Uses CLOCK_REALTIME synchronized to PTP via phc2sys.
 *
 * No custom hardware required — stock RPi5 with any PTP-capable NIC.
 *
 * Usage:
...
int main(int argc, char *argv[])
{
	...
	if (use_alsa)
		fprintf(stderr, "Waiting for AAF stream (ptime_offset=%u us, "
			"tolerance=%u us, AF_PACKET)...\n",
			ptime_offset_us, ptime_tolerance_us);
	else
